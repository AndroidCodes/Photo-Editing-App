...
public class MultiTouchController<T> {
    ...
    public static class PointInfo {
        ...
        private void set(int numPoints, float[] x, float[] y, float[] pressure, int[] pointerIds, int action, boolean isDown, long eventTime) {
            ...
            this.isDown = isDown;
            if (numPoints >= MultiTouchController.MODE_PINCH) {
                z = true;
            } else {
                z = false;
            }
    ...
    private void extractCurrPtInfo() {
        ...
        if (this.mCurrXform.updateScaleXY) {
            multiTouchDiameter = this.mCurrPt.getMultiTouchWidth();
        } else {
            multiTouchDiameter = 0.0f;
        }
        this.mCurrPtWidth = Math.max(MIN_MULTITOUCH_SEPARATION, multiTouchDiameter);
        if (this.mCurrXform.updateScaleXY) {
            multiTouchDiameter = this.mCurrPt.getMultiTouchHeight();
        } else {
            multiTouchDiameter = 0.0f;
        }
        this.mCurrPtHeight = Math.max(MIN_MULTITOUCH_SEPARATION, multiTouchDiameter);
        ...
        try {
            ...
            while (histIdx <= histLen) {
                ...
                if (!multiTouchSupported || pointerCount == MODE_DRAG) {
                    ...
                    fArr = xVals;
                    if (processingHist) {
                        historicalX = event.getHistoricalX(histIdx);
                    } else {
                        historicalX = event.getX();
                    }
                    fArr[MODE_NOTHING] = historicalX;
                    fArr = yVals;
                    if (processingHist) {
                        historicalX = event.getHistoricalY(histIdx);
                    } else {
                        historicalX = event.getY();
                    }
                    fArr[MODE_NOTHING] = historicalX;
                    fArr = pressureVals;
                    if (processingHist) {
                        historicalX = event.getHistoricalPressure(histIdx);
                    } else {
                        historicalX = event.getPressure();
                    }
                    fArr[MODE_NOTHING] = historicalX;
                } else {
                ...
                boolean z = processingHist ? true : (action == MODE_DRAG || (((MODE_DRAG << ACTION_POINTER_INDEX_SHIFT) - 1) & action) == ACTION_POINTER_UP || action == MODE_ST_GRAB) ? false : true;
                if (processingHist) {
    ...
    private void anchorAtThisPositionAndScale() {
        if (this.selectedObject != null) {
            ...
            float access$3 = !this.mCurrXform.updateScale ? 1.0f : this.mCurrXform.scale == 0.0f ? 1.0f : this.mCurrXform.scale;
            float currScaleInv = 1.0f / access$3;
            extractCurrPtInfo();
    ...
    private void performDragOrPinch() {
        ...
        if (this.selectedObject != null) {
            ...
            float deltaX = this.mCurrPt.getX() - this.mPrevPt.getX();
            float deltaY = this.mCurrPt.getY() - this.mPrevPt.getY();
            float newScale = this.mCurrXform.scale;
            if (this.mMode == MODE_ST_GRAB) {
                if (deltaX < 0.0f || deltaY < 0.0f) {
                    newScale = this.mCurrXform.scale - 0.04f;
                } else {
                    newScale = this.mCurrXform.scale + 0.04f;
                }
                if (newScale < 0.35f) {
            ...
            newScale = this.startScaleOverPinchDiam * this.mCurrPtDiam;
            if (this.mDragOccurred || pastThreshold(Math.abs(deltaX), Math.abs(deltaY), newScale)) {
                this.mCurrXform.set(newPosX, newPosY, newScale, this.startScaleXOverPinchWidth * this.mCurrPtWidth, this.startScaleYOverPinchHeight * this.mCurrPtHeight, this.startAngleMinusPinchAngle + this.mCurrPtAng);
    ...
    private boolean pastThreshold(float deltaX, float deltaY, float newScale) {
        if (deltaX >= THRESHOLD || deltaY >= THRESHOLD || newScale != this.mCurrXform.scale) {
            this.mDragOccurred = true;
            return true;
        }
        this.mDragOccurred = false;
        ...
        switch (this.mMode) {
            case MODE_NOTHING /*0*/:
                if (this.mCurrPt.isDown()) {
                    ...
                    long eventTime;
                    if (this.objectCanvas.pointInObjectGrabArea(this.mCurrPt, this.selectedObject)) {
                        this.mMode = MODE_ST_GRAB;
                        this.objectCanvas.selectObject(this.selectedObject, this.mCurrPt);
                        anchorAtThisPositionAndScale();
                        eventTime = this.mCurrPt.getEventTime();
